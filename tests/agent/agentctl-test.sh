...
assert_absent 'worktree 未落在仓库内' "$REPO/.agent-worktrees"
assert_exists 'task-a 元数据已写入' "$REPO/.agent/tasks/task-a.json"
assert_exists 'task-a 分支已创建' "$REPO/.git/refs/heads/agent/codex/task-a"
[[ "$(json_get "$REPO/.agent/tasks/task-a.json" platform)" == backend ]] && pass '元数据 platform=backend' || fail '元数据 platform=backend'
[[ "$(json_get "$REPO/.agent/tasks/task-a.json" status)" == active ]] && pass '元数据 status=active' || fail '元数据 status=active'
[[ "$(json_get "$REPO/.agent/tasks/task-a.json" allowed_paths.0)" == 'src/shared/**' ]] && pass '元数据 allowed_paths 落盘' || fail '元数据 allowed_paths 落盘'
[[ -n "$(json_get "$REPO/.agent/tasks/task-a.json" uuid)" ]] && pass '元数据含 UUID' || fail '元数据含 UUID'
[[ -n "$(json_get "$REPO/.agent/tasks/task-a.json" objective)" ]] && pass '元数据含 objective' || fail '元数据含 objective'

assert_file_contains 'TASK.md 含 Task ID' "$WTROOT/backend/task-a/.agent/TASK.md" 'Task ID: task-a'
assert_file_contains 'TASK.md 含 Platform' "$WTROOT/backend/task-a/.agent/TASK.md" 'Platform: backend'
assert_file_contains 'TASK.md 含 Objective' "$WTROOT/backend/task-a/.agent/TASK.md" '为 backend 增加 A 功能'
assert_file_contains 'TASK.md 含 Requirements' "$WTROOT/backend/task-a/.agent/TASK.md" '- 补充测试'
assert_file_contains 'TASK.md 含 Scope' "$WTROOT/backend/task-a/.agent/TASK.md" 'src/shared/**'
assert_file_contains 'TASK.md 含 Restrictions' "$WTROOT/backend/task-a/.agent/TASK.md" '## Restrictions'
assert_file_contains 'TASK.md 含 Completion Criteria' "$WTROOT/backend/task-a/.agent/TASK.md" '## Completion Criteria'
assert_file_contains 'TASK.md 提示不要并入平台分支' "$WTROOT/backend/task-a/.agent/TASK.md" '不并入平台分支'
...
gw "$WTROOT/backend/task-a" check-ignore -q .agent/TASK.md && pass 'TASK.md 已被 info/exclude 排除' || fail 'TASK.md 已被 info/exclude 排除'
g check-ignore -q .agent/tasks 2>/dev/null && pass '任务注册表已被仓库 .gitignore 排除' || fail '任务注册表已被仓库 .gitignore 排除'
...
[[ -n "$(json_get "$REPO/.agent/tasks/web-google-oauth-001.json" id)" ]] &&
...
assert_absent '被拒绝后不留下任务记录' "$REPO/.agent/tasks/task-y.json"
...
[[ "$(json_get "$REPO/.agent/tasks/task-a.json" status)" == active ]] && pass 'finish 失败不改状态' || fail 'finish 失败不改状态'
...
[[ "$(json_get "$REPO/.agent/tasks/task-a.json" status)" == ready ]] && pass 'finish 后状态 ready' || fail 'finish 后状态 ready'

section '10. task set-status（状态机）'
run task set-status task-z blocked --note '等待上游接口'
assert_rc 'set-status blocked' 0
[[ "$(json_get "$REPO/.agent/tasks/task-z.json" status)" == blocked ]] && pass '状态已落盘 blocked' || fail '状态已落盘 blocked'
...
[[ "$(json_get "$REPO/.agent/tasks/task-a.json" status)" == merged ]] && pass 'integrate 后状态 merged' || fail 'integrate 后状态 merged'
[[ "$(json_get "$REPO/.agent/tasks/task-a.json" merged_into)" == backend ]] && pass '记录 merged_into=backend' || fail '记录 merged_into=backend'
...
assert_absent 'merge-check 未残留临时 worktree' "$REPO/.agent/state/tmp"
...
[[ "$(json_get "$REPO/.agent/tasks/task-b.json" status)" == conflict ]] && pass '冲突时状态置 conflict' || fail '冲突时状态置 conflict'
run task integrate task-b
...
[[ "$(json_get "$REPO/.agent/tasks/task-b.json" status)" == merged ]] && pass 'task-b 状态 merged' || fail 'task-b 状态 merged'
...
[[ "$(json_get "$REPO/.agent/tasks/backend-parallel-one-001.json" agent)" == codex &&
  "$(json_get "$REPO/.agent/tasks/backend-parallel-two-001.json" agent)" == omp ]] &&
...
assert_absent '任务记录已删除' "$REPO/.agent/tasks/task-a.json"
printf 'leftover\n' >"$WTROOT/backend/task-z/src/index.ts"
...
[[ "$(json_get "$REPO/.agent/tasks/api-alias-task-001.json" platform)" == api ]] && pass '任务记录使用声明平台 id' || fail '任务记录使用声明平台 id'
[[ "$(json_get "$REPO/.agent/tasks/api-alias-task-001.json" test_command)" == '' ]] && pass '未显式指定时 test_command 走平台配置' || fail '未显式指定时 test_command 走平台配置'
